...
```java
CreateWebhookRequest body = new CreateWebhookRequest.Builder()
    .namespace(WebhookNamespaces.PAYMENTQUOTES_ACCEPTED)
    .url("https://fawebhookreceiver.azurewebsites.net/api/receive")
    .build();

webhooksController.createSubscriptionAsync(body).thenAccept(result -> {
...
```java
String webhookToken = "webh-6260c132-5cb1-4e30-8b08-9ce559893acb";

webhooksController.deleteSubscriptionAsync(webhookToken).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
webhooksController.listSubscriptionsAsync(page, pageSize, filter, sort, language).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
webhooksController.retrieveSubscriptionAsync(webhookToken).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
webhooksController.updateSubscriptionAsync(body).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
