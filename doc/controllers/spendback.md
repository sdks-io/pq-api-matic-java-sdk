...
spendbackController.acceptSpendbackQuoteAsync(spendbackToken).thenAccept(result -> {
...
spendbackController.cancelSpendbackQuoteAsync(spendbackToken).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
| Parameter | Type | Tags | Description |
|  --- | --- | --- | --- |
| `body` | [`SpendbackQuote`](../../doc/models/containers/spendback-quote.md) | Body, Optional | - |
...
```java
SpendbackQuote body = SpendbackQuote.fromGatewaySpendbackQuote(
    new GatewaySpendbackQuote.Builder()
        .amount(1.02D)
...
spendbackController.createSpendbackQuoteAsync(body).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
spendbackController.listSpendbacksAsync(page, pageSize, filter, sort, language).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
...
spendbackController.retrieveSpendbackAsync(spendbackToken, page, pageSize, filter, sort, language).thenAccept(result -> {
    // TODO success callback handler
    System.out.println(result);
}).exceptionally(exception -> {
    Throwable cause = exception.getCause();
